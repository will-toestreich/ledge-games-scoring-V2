...
import { events } from "@/data/competition-config";
import { useSettings } from "@/data/hooks";
import { isScorerAuthed, markScorerAuthed } from "@/lib/scorer-auth";

/**
 * Wraps every scoring route: shows the PIN pad until the scorer unlocks,
 * then renders children. Session-scoped, so deep links stay locked too.
 */
export function ScorerGate({ children }: { children: React.ReactNode }) {
  const [authed, setAuthed] = useState(isScorerAuthed());
  ...
  function handlePinSubmit() {
    if (settings && pinEntry === settings.scorerPin) {
      markScorerAuthed();
      setAuthed(true);
    } else {
      setShake(true);
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-3.5rem)]">
      ...
      >
            ...
            {[0, 1, 2, 3].map((i) => (
              <div
                ...
                className={`w-12 h-14 rounded-xl border-2 flex items-center justify-center text-2xl font-bold transition-all duration-200 ${
                  pinEntry[i]
                    ? "border-ledge-red bg-ledge-red/10 text-text-primary"
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          ...
          <button onClick={() => setPinEntry("")} className="numpad-key py-3.5 text-sm text-text-tertiary">
            Clear
          </button>
          <button
          ...
          <button onClick={() => setPinEntry((p) => p.slice(0, -1))} className="numpad-key py-3.5 text-text-tertiary">
          ...
          {events.map((event) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-lg text-text-primary transition-colors">
                  {event.name}
                </h3>
                <p className="text-sm text-text-tertiary mt-0.5">
                  {event.format === "ladder"
                    ? `bar starts at ${event.ladder!.startHeight} ${event.unit}`
                    : `${event.unit} · ${event.direction === "asc" ? "lowest wins" : "highest wins"}`}
                </p>
              </div>
